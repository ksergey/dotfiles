...
notify-send "Getting list of available Wi-Fi networks..." --app-name="${app}" --icon="${icon}"
...
if [ -z "$chosenNetwork" ]; then
    ...
    nmcli radio wifi on
    notify-send "Wi-Fi Status" "Wi-Fi enabled." --app-name="${app}" --icon="${icon}"
elif [ "$chosenNetwork" = "󰖪  Disable Wi-Fi" ]; then
    nmcli radio wifi off
    notify-send "Wi-Fi Status" "Wi-Fi disabled." --app-name="${app}" --icon="${icon}"
else
    ...
    if [[ $(echo "$savedConnections" | grep -w "$chosenId") = "$chosenId" ]]; then
        nmcli connection up id "$chosenId" | grep "successfully" && notify-send "Connection Established" "$successMessage"
    else
        ...
        if nmcli device wifi connect "$chosenId" password "$wifiPassword"; then
            notify-send "Connection Established" "$successMessage" --app-name="${app}" --icon="${icon}"
        else
            notify-send "Connection Failed" "Could not connect to \"$chosenId\". Please check your password or network." --app-name="${app}" --icon="${icon}"
        fi
    fi
fi
